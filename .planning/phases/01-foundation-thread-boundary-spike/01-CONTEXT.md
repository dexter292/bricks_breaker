...
- **D-18:** Use **Node 24 LTS** via nvm/fnm. Pin version in **`.nvmrc`** and enforce via package **`engines`**. Vitest `core/` smoke test remains a mandatory Phase 1 completion criterion.
...
- Whether to use nvm or fnm as long as `.nvmrc` + `engines` are enforced
