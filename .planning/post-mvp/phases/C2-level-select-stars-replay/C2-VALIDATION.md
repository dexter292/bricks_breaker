...
|--------|----------|-----------|-------------------|--------------|
| N-PROG-03 | `computeStars` clamp 1–3 | unit | `npx vitest run tests/storage.progress-v3.test.ts` | ❌ W0 |
| N-PROG-03 | Win max(stars); lose no stars write | unit | same | ❌ W0 |
| N-PROG-03 | v1→v3 / v2→v3 preserve unlocked + scores | unit | same | ❌ W0 |
| N-PROG-03 | Corrupt v3 → defaults; watermark safe | unit | same | ❌ W0 |
| N-LVL-02 | Select three row states + locked ignore + mount snapshot | UI | `npx vitest run tests/ui/SelectScreen.test.tsx` | ❌ W0 |
| N-LVL-02 | Title → Select → Playing → Menu → Title | UI | `npx vitest run tests/ui/GameHost.test.tsx` | ⚠️ update |
| N-PROG-04 | Win Next gated; hidden on level-06; lose no Next | UI | ResultOverlay / GameScreen tests | ❌ W0 |
| Bake D-03 | `setActive(true)` last after levelId change | UI/unit | `npx vitest run tests/ui/PlayingHost.next-bake.test.ts` | ❌ W0 |
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| C2-W0-* | 00 | 0 | N-* | T-C2-* | Stubs / contracts | unit/UI stub | quick run | ❌ | ⬜ pending |

*Filled by planner when PLAN.md tasks land.*

---

## Wave 0 Requirements

- [ ] `tests/storage.progress-v3.test.ts` — migrate, stars merge, corrupt
- [ ] `src/services/storage/stars.ts` (or equiv) — `computeStars` / merge / row state
- [ ] `tests/ui/SelectScreen.test.tsx`
- [ ] `tests/ui/PlayingHost.next-bake.test.ts` — setActive-last
- [ ] Update `tests/ui/GameHost.test.tsx` for Select
- [ ] Update `tests/runtime.loadLevel.test.ts` — no default id
- [ ] ResultOverlay / GameScreen Next + stars coverage
