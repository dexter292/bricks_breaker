...
|------|-------|---------|----|--------------|-------|
| Android FPS gate (D-01) | _TBD — Pixel 6a or D-04 substitute_ | | | | |
| iOS install/feel (D-02) | _TBD — physical iPhone_ | | | | |
...
| iOS | development | EAS | _blocked_ | | D-17: no credentials suitable for internal distribution |
| iOS | profiling | EAS | _blocked_ | | Same credential blocker |

**Build path summary:** Android via EAS (`development` + `profiling`) — both **finished** with Skia 2.12.0 (no fallback needed for Android build). iOS blocked pending paid ADP distribution cert + provisioning for internal distribution (D-17). Project `@dexter292/bricks-breaker` (`9f72de84-a221-46f1-8afd-209bda078d86`). Bundle id / package: `com.dexter292.bricksbreaker`.
...
| Issue | Disposition |
|-------|-------------|
| iOS EAS credentials missing for internal distribution | STOP — human-action (D-17). Do not use free personal team / 7-day profiles. |

---

_Status: stub created in plan 04 Task 1 — fill after on-device gates (Task 2)._
